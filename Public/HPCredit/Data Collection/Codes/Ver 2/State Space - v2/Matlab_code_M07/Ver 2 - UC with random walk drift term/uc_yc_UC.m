...
data_im = dlmread('D:\GitHub\HPCredit\Data Collection\Codes\Ver 2\State Space - v2\data_US.txt',',',1,1);
...
[xout,fout,cout,output,gout,hout] = ...
    fminunc(@(prmtr)lik_fcnUC(prmtr,y,T,START,t_y_prior,t_h_prior),prmtr_in,options);
%Returns paramter estimates, -LL value, code

%Final parameter values
prm_fnl = transUC(xout);
...
par = sym('p',[12 1]);
grdn_fnl = jacobian(transUC(par),par); 
...
results = fopen("results_US_UC.txt",'w');
...
writetable(prmtr_in_table, 'prmtr_in_US_UC.txt','WriteVariableNames',0)
type 'prmtr_in_US.txt'
%Write file to a specific folder
my_directory = 'D:\OnlineDrive\OneDrive\Project\HPCredit\Paper';  
writedata = [my_directory filesep 'Data_US_UC.csv'];
...
[data,forcst] = filter_fcnUC(xout,y,T,START,t_y_prior,t_h_prior);

%Creates output file to store filtered dataset
csvwrite("uc_yc_US_UC.txt",[data(:,1),data(:,2),data(:,4),data(:,5),forcst(:,1:2)]);
