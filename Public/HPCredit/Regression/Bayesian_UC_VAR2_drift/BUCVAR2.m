...
for j=3:T
    ...
    cy(j,1)=cy(j-1,1)*F(2,2)+cy(j-2,1)*F(2,3)+e(j,2)*cQ(2,2);
    th(j,1)=mu+th(j-1,1)+e(j,3)*cQ(3,3);
    ch(j,1)=ch(j-1,1)*F(5,5)+ch(j-2,1)*F(5,6)+e(j,4)*cQ(4,4);
    y(j)=ty(j,1)+cy(j,1);
    h(j)=th(j,1)+ch(j,1);
end
...
B=cov(e(:,2),e(:,4));
B=B(1,2);
TRUE=[F(2,2);F(2,3);F(5,5);F(5,6);ones(4,1)*0.6;Q(3,1);Q(4,2)];
