...
ggplot(df6, aes(date, NFECredit_GDP_gap, color = ID)) +
  geom_hline(yintercept = 0, linetype = "dashed",
             color = "grey70", size = 0.02) +
  geom_line(show.legend = FALSE) +
  facet_wrap(~ID) +
  ...
  theme(panel.grid = element_blank()) +
  labs(x = NULL, y = NULL,
       title = "Non Financial Enterprise Credit",
       subtitle = "as percentage of GDP - HP decomp cycle | lambda=400000")
