clear, clc
working_dir = '/Users/namnguyen/Documents/GitHub/HPCredit/Regression/Bayesian_UC_AR2/Reference Codes - Applied Bayesian Chap 5'
cd(working_dir)
...
posterior(theta0,y,x,F0,VF0,MU0,VMU0,R0,VR0,Q0,VQ0,1)

% % %simplex
 [Theta1,fval] = fminsearch(@posterior, theta0,options,y,x,F0,VF0,MU0,VMU0,R0,VR0,Q0,VQ0,1);
 %****************************
 

%***************step 2 estimate model via maximum likelihood
Theta0=ones(7,1).*0.2;
[FF,AA,gh,hess,itct,fcount,retcodeh] = csminwel('posterior',Theta1,eye(length(Theta1))*.1,[],1e-15,1000,y,x,F0,VF0,MU0,VMU0,R0,VR0,Q0,VQ0,1);
...
%compute posterior at old draw
        ...
        jj=1;
for j=1:REPS
    % Print steps number and temporary estimates
    if 0 == mod(j, 10000)
      disp(j)
      disp(Gammaold)
    end
