clear, clc
working_dir = '/Users/namnguyen/Documents/GitHub/HPCredit/Regression/Bayesian_UC_AR2_nodrift'
cd(working_dir)
...
theta0(3) = priors_trend_stddev(1);
theta0(4) = priors_VAR2(5);
...
for j=1:REPS   
    ...
    Gammanew=Gammaold+(randn(1,4)*chol(K*P))';
    
    %step 2 check elements of D are positive, variances positive and
    %elements of F sum to less than 1
    check=sum([sum(Gammanew(3:end)<0)  sum(abs(Gammanew(1:2))>2) ...
        && (sum(Gammanew(1:2))>=0.98) (Gammanew(1)<0.4) ...
        (Gammanew(3)>1) (Gammanew(4)>1)]);
    if check
