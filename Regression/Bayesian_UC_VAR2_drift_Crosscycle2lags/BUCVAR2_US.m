...
data_im = readtable(input_filepath);
%data_im[:,1] = [];
startvalues=zeros(1,6);
startvalues(1)=data_im{2,7};
startvalues(2)=data_im{2,8};
startvalues(3)=data_im{1,8};
startvalues(4)=data_im{2,9};
startvalues(5)=data_im{2,10};
startvalues(6)=data_im{1,10};
startvalues(7)=0;
startvalues(8)=0;
...
theta0(9) = priors_trend_stddev(1);
theta0(10) = priors_VAR2x(17);
theta0(11) = priors_trend_stddev(2);
...
%        sum(Gammanew(5:8)>2)]);
    check = sum(Gammanew(9:12)<0) && sum(abs(Gammanew(1:8))>2) ...
        && (sum(Gammanew(1:4))>=2) ...
        && (sum(Gammanew(1:2))>=1.8) && (sum(Gammanew(3:4))<=-0.8)...
        && (sum(Gammanew(5:8))>=2) ...
        && (sum(Gammanew(7:8))>=1.8) && (sum(Gammanew(5:6))<=-0.8)...
        && (Gammanew(1)<0.5) && (Gammanew(7)<0.5) ...
        && sum(Gammanew(9:12)>6) ...
        && (Gammanew(14)/sqrt(Gammanew(10)*Gammanew(12)))>=1;
    if check
...
X=0.5:(1/25):2.5;
X2=X-(1/25);
Y=normcdf(X,theta0(7),0.5)-normcdf(X2,theta0(7),0.5);
plot(X,Y);
hold off
...
X=-1.5:(1/25):0.5;
X2=X-(1/25);
Y=normcdf(X,theta0(8),0.5)-normcdf(X2,theta0(8),0.5);
plot(X,Y);
hold off
...
title('\sigma_{nynh}');
hold on
histogram(out1(:,13), 50, 'Normalization','probability');
X=0.04:(1/25):2.04;
X2=X-(1/25);
Y1=gammainc(2./X,2,'upper');
Y2=gammainc(2./X2,2,'upper');
...
title('\sigma_{eyeh}');
hold on
histogram(out1(:,14), 50, 'Normalization','probability');
X=0.04:(1/25):2.04;
X2=X-(1/25);
Y1=gammainc(2./X,2,'upper');
Y2=gammainc(2./X2,2,'upper');
...
for j=1:(REPS-BURN)
 stddev1(j)=sqrt(out1(j,9));
 stddev2(j)=sqrt(out1(j,10));
 stddev3(j)=sqrt(out1(j,11));
 stddev4(j)=sqrt(out1(j,12));
 corr1(j)=(out1(j,13)/sqrt(out1(j,9)*out1(j,11)));
 corr2(j)=(out1(j,14)/sqrt(out1(j,10)*out1(j,12)));
end
